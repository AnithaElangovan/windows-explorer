...
public class WindowsExplorerController 
{
	...
	public ModelAndView frontpage(@ModelAttribute ("Explorer")Explorer explorer,BindingResult result) 
	{
		if (tempVariable < 1) {
			tempVariable++;
...
//			explorer.addListOfDrives(desktop);
		...
		ModelAndView model = new ModelAndView("redirect:/display"+(i++));
		return model;
	}
	...
	public ModelAndView getIndex(){
		...
		pm = PersitenceManagerFactoryClass.get().getPersistenceManager();
         ...
         ModelAndView model = null;
			try
			{
				String driveFolderObject = null;
				String folderFileObject = null;
...
//		for (int i = 0; i < tempList.size(); i++) 
//		{
		model = new ModelAndView("redirect:/display");
...
//		}
		...
		public ModelAndView foldercreated(@ModelAttribute ("folder")Folder folder,@RequestParam ("folderName")String folderName,@RequestParam ("folderType")String folderType,BindingResult result,ModelMap model1){
			...
			ModelAndView model = new ModelAndView("redirect:/display");
			return model;
		}
		...
		public String copyfile(HttpServletResponse resp,@RequestParam("fileName") String fileName,@RequestParam("folderName") String folderName,ModelMap model) throws IOException {
